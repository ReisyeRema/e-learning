        ...
        <div class="row">
            ...
            <aside class="col-md-2 sidebar d-flex flex-column align-items-start">
                ...
                <div class="d-flex align-items-center bg-white text-dark p-2 rounded mb-4 w-100">
                    <img src="{{ $user->foto ? asset('storage/foto_user' . $user->foto) : asset('assets/img/profil.png') }}"
                        class="profile-pic me-2" alt="{{ $user->name }}">
                    <div class="overflow-hidden">
                        <div class="fw-semibold text-truncate" style="max-width: 200px;">{{ $user->name }}</div>
                        <small class="text-truncate d-block" style="max-width: 200px;">{{ $user->email }}</small>
                    </div>
                </div>
                ...
                <div class="w-100 mb-4">
                    ...
                    <ul class="nav flex-column w-100">
                        <li class="nav-item">
                            <a class="nav-link hover-effect {{ $isActive ? 'active-sidebar' : 'text-white' }} text-center"
                                href="{{ $listUrl }}">
                ...
                <div class="w-100 mb-4">
                    ...
                    <div class="text-white small">
                        <p><i class="bi bi-chat-left-text me-2"></i>Total Diskusi:
                            <strong>{{ $pembelajaran->forum ? $pembelajaran->forum->count() : 0 }}</strong></p>
                        <p><i class="bi bi-people-fill me-2"></i>Anggota Aktif:
                            <strong>{{ 1 + ($pembelajaran->enrollments ? $pembelajaran->enrollments->count() : 0) }}</strong>
                        </p>
                    </div>
                </div>
            ...
            <main class="col-md-8 p-4">
                <div class="card p-4 shadow-sm">
                    <h4 class="mb-4">Forum Diskusi Kelas {{ $pembelajaran->kelas->nama_kelas }} -
                        {{ $pembelajaran->nama_mapel }}</h4>
                    @yield('content')
            ...
            <aside class="col-md-2 right-panel">
                ...
                <div class="bg-white text-dark rounded-3 p-1 mb-3 shadow-sm d-flex align-items-center follow-card">
                    <img src="{{ $pembelajaran->guru->foto ? asset('storage/foto_user' . $pembelajaran->guru->foto) : asset('assets/img/profil.png') }}"
                        class="me-2" alt="guru">
                    <div class="overflow-hidden">
                        <div class="fw-bold text-truncate" style="max-width: 200px;">{{ $pembelajaran->guru->name }}
                        </div>
                ...
                @foreach ($pembelajaran->enrollments as $enrollment)
                    @if ($enrollment->siswa)
                        <div
                            ...
                            <img src="{{ $enrollment->siswa->foto ? asset('storage/foto_user' . $enrollment->siswa->foto) : asset('assets/img/profil.png') }}"
                                class="me-2" alt="{{ $enrollment->siswa->name }}">
                            <div class="overflow-hidden">
                                <div class="fw-bold text-truncate" style="max-width: 200px;">
                                    {{ $enrollment->siswa->name }}</div>
